...
# Step 2: Start nginx
log "Starting nginx..."
docker compose -f "$APPS_COMPOSE" --env-file "$ENV_FILE" up -d nginx

# Step 3: Get real certificate
log "Requesting real certificate for ${DOMAIN}..."
docker compose -f "$APPS_COMPOSE" --env-file "$ENV_FILE" run --rm certbot \
    certbot certonly \
    --webroot \
...
docker compose -f "$APPS_COMPOSE" --env-file "$ENV_FILE" exec nginx nginx -s reload
